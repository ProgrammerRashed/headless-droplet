import PlainRichTextContainer from "@/components/shared/PlainRichTextContainer";
import { getPageContent } from "@/graphql/Components/getPageContent";
import { getSeoData } from "@/graphql/Components/getSeoData";

export async function generateMetadata() {
  const data = await getSeoData("terms-and-condition") || {};
...
const TermsAndCondition = async () => {
    const pageContent = await getPageContent("terms-and-condition")
    
  return (
    <div className="mt-40">
        <PlainRichTextContainer data={pageContent}/>
    </div>
  )
}
