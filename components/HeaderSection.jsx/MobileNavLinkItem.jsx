  ...
  return (
    ...
    >
      <AccordionTrigger
        ...
        className="py-[15px] text-lg font-medium leading-[28px] tracking-[-1.5%] text-gray-800 hover:no-underline"
      >
        ...
        <div className="mb-[10px] flex w-full flex-col gap-3 rounded-[8px] bg-surface p-[15px] text-base font-medium leading-[22px] text-gray-600">
          {navigation.childNavigations.map((navItem) => (
            <Link
              ...
              key={navItem.id}
              className="transition-all duration-300 hover:text-gray-800 hover:underline"
              href={navItem.link}
            >
              {navItem.title}
            </Link>
