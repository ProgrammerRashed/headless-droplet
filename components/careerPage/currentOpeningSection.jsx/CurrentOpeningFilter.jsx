...
function CurrentOpeningFilter({ departments, country, selectedDepartment, selectedLocation, onDepartmentChange, onLocationChange }) {
  useEffect(() => {
  ...
  return (
    <form className="flex max-w-sm flex-grow items-end justify-end gap-[10px] md:max-w-md 0.5xl:max-w-[580px]">
      <div data-aos="fade-up" className="w-full flex-col gap-1">
        <Select value={selectedLocation} onValueChange={onLocationChange} className="w-full">
          <SelectTrigger
            id="country"
      ...
      <div data-aos="fade-up" className="w-full flex-col gap-1">
        <Select value={selectedDepartment} onValueChange={onDepartmentChange} className="w-full">
          <SelectTrigger
            id="department"
            className="m-0 h-auto w-full rounded-[40px] border border-gray-50 bg-white px-4 py-3 text-sm leading-[22px] text-gray-700 shadow-none placeholder:text-gray-400 focus:ring-green-500"
          >
